...
use zoeyr_wire_protocol::{
    MessageFilters, MessageFull, Message, Kind, Tag, RelayError, StreamRequest,
};
...
async fn send_messages(
    ...
    loop {
        let timestamped_message = if follow {
            format!("{} #{} - {}", message, counter, chrono::Utc::now().format("%H:%M:%S"))
        } else {
            message.clone()
        };
        ...
        let message_data = test_message.storage_value()
        ...
        match client.relay_service().await {
            Ok(relay_service) => {
                match relay_service.store_message(tarpc::context::current(), message_data).await {
                    Ok(Ok(stream_id)) => {
...
async fn main() -> Result<()> {
    ...
    info!("📋 Client public key: {}", hex::encode(client.client_public_key()));
    ...
    if cli.follow {
        ...
        let listen_task = tokio::spawn(async move {
            listen_for_messages(listen_client, true).await
        });
        ...
        send_messages(Arc::clone(&client), Arc::clone(&client_key), cli.message, send_interval, false).await?;
