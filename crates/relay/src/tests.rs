...
use zoeyr_wire_protocol::{
    StreamProtocolMessage, StreamRequest, MessageFilters, StreamResponse, StreamingMessage,
};
...
async fn test_streaming_protocol_in_process() -> Result<()> {
    ...
    let server_task = tokio::spawn(async move {
        ...
        let stream_msg: StreamProtocolMessage = postcard::from_bytes(&first_frame)
            .expect("Should be valid streaming protocol message");
    ...
    match message {
        StreamProtocolMessage::Message(StreamingMessage::MessageReceived { 
            ...
            println!("📨 Client received message: {}", String::from_utf8_lossy(&message_data));
        }
...
async fn test_tarpc_framing_debug() -> Result<()> {
    ...
    println!("🔍 Postcard serialized ({} bytes): {:?}", serialized.len(), serialized);
    ...
    println!("📥 Received frame ({} bytes): {:?}", received_frame.len(), received_frame);
    ...
    println!("🔍 Streaming protocol detection result: {:?}", stream_attempt.is_err());
    assert!(stream_attempt.is_err(), "Should not deserialize as streaming protocol");
...
fn test_protocol_detection_logic() -> Result<()> {
    ...
    let detected_not_stream: Result<StreamProtocolMessage, _> = postcard::from_bytes(&not_streaming_bytes);
    assert!(detected_not_stream.is_err(), "Should not detect as streaming protocol");
...
async fn test_stream_with_first_frame_integration() -> Result<()> {
    ...
    println!("Server read first frame: {:?}", String::from_utf8_lossy(&first_frame));
    ...
    println!("✅ First frame restored correctly: {:?}", String::from_utf8_lossy(&restored_frame));
...
async fn test_tarpc_transport_compatibility() -> Result<()> {
    ...
    println!("🚨 ISSUE IDENTIFIED: tarpc serde_transport cannot accept StreamWithFirstFrame wrapper");
...
async fn test_tarpc_solution_with_buffered_stream() -> Result<()> {
    ...
    let buffered_stream = crate::server::StreamWithBufferedData::new(server_stream, first_frame_data);
    let codec = LengthDelimitedCodec::new();
