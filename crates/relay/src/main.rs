...
async fn main() -> Result<()> {
    ...
    if matches.get_flag("generate-key") {
        ...
        println!("You can use this key in your configuration file or set it via environment variable.");
        return Ok(());
    }
    ...
    info!("Server public key: {}", hex::encode(config.server_key.verifying_key().to_bytes()));
    ...
    let router = RelayServiceRouter::new(blob_service);
...
mod tests {
    ...
    fn test_load_config() {
        ...
        assert_eq!(loaded_config.blob_config.data_dir, PathBuf::from("/test/path"));
    }

    #[test]
    fn test_default_config() {
        let config = RelayConfig::default();
        assert_eq!(config.blob_config.data_dir, PathBuf::from("./blob-store-data"));
    }
}