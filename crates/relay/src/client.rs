...
impl QuicTarpcClient {
    ...
    pub async fn relay_service(&self) -> Result<RelayServiceClient> {
        ...
        use crate::server::{QuicDuplexStream, PostcardSerializer};
    ...
    pub async fn start_message_stream(&self, stream_request: StreamRequest) -> Result<()> {
        ...
        framed.send(request_bytes.into())
            .await
        ...
        info!("✅ Streaming completed, received {} messages", message_count);
        Ok(())
    }
}
...
impl RelayClientBuilder {
    ...
    pub async fn build(self) -> Result<QuicTarpcClient> {
        ...
        let client = QuicTarpcClient::connect(
            self.server_addr,
            expected_server_key,
            client_key,
        ).await?;
