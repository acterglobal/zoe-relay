        ...
        match extract_ed25519_from_cert(end_entity) {
            Ok(cert_ed25519_key) => {
                if cert_ed25519_key.to_bytes() == self.expected_server_ed25519_key.to_bytes() {
                    ...
                    error!("   Expected: {}", hex::encode(self.expected_server_ed25519_key.to_bytes()));
            ...
            Err(e) => {
                error!("❌ Failed to extract ed25519 key from server certificate: {}", e);
                Err(rustls::Error::InvalidCertificate(
                    rustls::CertificateError::ApplicationVerificationFailure
                ))
            }
...
impl RelayClient {
    ...
    pub async fn connect(
        ...
        info!("🔑 Expected server ed25519 key: {}", hex::encode(expected_server_ed25519_key.to_bytes()));
        info!("🔑 Client ed25519 key: {}", hex::encode(client_key.verifying_key().to_bytes()));
        
        // Create client config with server identity verification
        let crypto = rustls::ClientConfig::builder()
            .dangerous()
            .with_custom_certificate_verifier(Arc::new(
                ServerEd25519TlsVerifier::new(expected_server_ed25519_key.clone())
            ))
            .with_no_client_auth();
    ...
    /// Send JSON-serialized data and receive response
    pub async fn send_json<T, R>(&self, request: &T) -> Result<R>
    where
        T: serde::Serialize,
        R: for<'a> serde::Deserialize<'a>,
    {
        ...
        let response: R = serde_json::from_str(&response_str)?;
...
pub fn create_relay_server_endpoint(
    addr: SocketAddr,
    server_key: &SigningKey,
) -> Result<Endpoint> {
    info!("🚀 Creating relay server endpoint");
    info!("📋 Server Address: {}", addr);
    info!("🔑 Server Public Key: {}", hex::encode(server_key.verifying_key().to_bytes()));
    ...
    info!("💡 Clients can connect with server public key: {}", hex::encode(server_key.verifying_key().to_bytes()));
    
    Ok(endpoint)
}

/// Parse a hex-encoded ed25519 public key
pub fn parse_ed25519_public_key(public_key_hex: &str) -> Result<VerifyingKey> {
    let key_bytes = hex::decode(public_key_hex.trim())
