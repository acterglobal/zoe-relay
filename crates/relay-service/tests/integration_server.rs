...
// Update imports to use merged crates
use zoeyr_relay_service::*; // Use relay-service crate
...
async fn integration_protocol_message_handling() {
    ...
    let json_str = serde_json::to_string(&message).expect("Serialization failed");
    let deserialized: ProtocolMessage<String> = serde_json::from_str(&json_str)
...
async fn integration_health_check_protocol() {
    ...
    // Deserialize both
    let check_deser: ProtocolMessage<String> = serde_json::from_str(&check_json).unwrap();
...
async fn integration_error_handling() {
    ...
    let json_str = serde_json::to_string(&error_message).unwrap();
...
async fn integration_message_full_creation() {
    ...
    let message_full = MessageFull::new(message, &signing_key).expect("Failed to create MessageFull");
    
    // Verify the message
    assert!(message_full.verify().expect("Verification failed"));
    assert!(message_full.verify_id().expect("ID verification failed"));
    assert!(message_full.verify_all().expect("Complete verification failed"));
}
