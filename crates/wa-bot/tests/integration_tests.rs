...
async fn test_qr_code_display_flow() -> Result<()> {
    ...
    assert!(qr_displayed, "QR code should be displayed for disconnected bot");
...
async fn test_message_filtering() -> Result<()> {
    ...
    // Test group filter
    assert!(should_display_message(&group_message, &None, &None, true, false));
    assert!(!should_display_message(&dm_message, &None, &None, true, false));
    
    // Test DM filter
    assert!(!should_display_message(&group_message, &None, &None, false, true));
    assert!(should_display_message(&dm_message, &None, &None, false, true));
    
    // Test sender filter
    assert!(should_display_message(&group_message, &Some("1234".to_string()), &None, false, false));
    assert!(!should_display_message(&group_message, &Some("9999".to_string()), &None, false, false));
...
fn test_jid_name_extraction() {
    ...
    assert_eq!(extract_name_from_jid("1234567890@s.whatsapp.net"), "+1234567890");
