...
//!
//! This module implements the actual Post-Quantum Extended Diffie-Hellman (PQXDH) 
...
//!
//! 1. **Prekey Generation**: Server generates and publishes prekey bundle
//! 2. **Initiation**: Client performs key agreement using server's prekeys
//! 3. **Response**: Server derives the same shared secret using private keys
//! 4. **Session**: Both parties use derived key for symmetric encryption
//!
//! ## Security Properties
//!
//! - **IND-CCA2**: Indistinguishability under adaptive chosen-ciphertext attack
//! - **Forward Secrecy**: Ephemeral keys provide forward secrecy
//! - **Post-Compromise Security**: One-time prekeys provide post-compromise security
...
use hkdf::Hkdf;
use libcrux_ml_kem::{MlKemPrivateKey, MlKemPublicKey};
...
pub fn generate_pqxdh_prekeys<R: CryptoRng + RngCore>(
    ...
    let (mlkem_signed_private, mlkem_signed_public) = libcrux_ml_kem::mlkem768::generate_key_pair(rng);
    let mlkem_signed_public_bytes = mlkem_signed_public.as_slice().to_vec();
    ...
    for i in 0..num_one_time_keys {
        let (otk_private, otk_public) = libcrux_ml_kem::mlkem768::generate_key_pair(rng);
        let otk_public_bytes = otk_public.as_slice().to_vec();
        ...
        mlkem_one_time_privates.insert(otk_id.clone(), otk_private.as_slice().to_vec());
    ...
    let private_keys = PqxdhPrivateKeys {
        ...
        signed_prekey_id: x25519_signed_prekey_id,
        one_time_prekey_privates: x25519_one_time_privates,
        pq_signed_prekey_private: mlkem_signed_private.as_slice().to_vec(),
        pq_signed_prekey_id: mlkem_signed_prekey_id,
...
pub fn pqxdh_initiate<R: CryptoRng + RngCore>(
    ...
    let mlkem_signed_public = MlKemPublicKey::from_slice(&prekey_bundle.pq_signed_prekey)
        .context("Invalid ML-KEM signed prekey")?;
    let (mlkem_signed_ciphertext, mlkem_signed_shared) = 
        libcrux_ml_kem::mlkem768::encapsulate(&mlkem_signed_public, rng);
    ...
    if let Some(otk_id) = &mlkem_one_time_key_id {
        if let Some(otk_public_bytes) = prekey_bundle.pq_one_time_keys.get(otk_id) {
            let otk_public = MlKemPublicKey::from_slice(otk_public_bytes)
                .context("Invalid ML-KEM one-time prekey")?;
            let (otk_ciphertext, otk_shared) = 
                libcrux_ml_kem::mlkem768::encapsulate(&otk_public, rng);
...
pub fn pqxdh_respond(
    ...
    // Verify that the prekey IDs match
    if initial_message.signed_prekey_id != private_keys.signed_prekey_id {
        return Err(anyhow::anyhow!("Signed prekey ID mismatch"));
    }
    if initial_message.pq_signed_prekey_id != private_keys.pq_signed_prekey_id {
        return Err(anyhow::anyhow!("PQ signed prekey ID mismatch"));
    }
    ...
    let mlkem_signed_private = MlKemPrivateKey::from_slice(&private_keys.pq_signed_prekey_private)
        .context("Invalid ML-KEM signed private key")?;
    let signed_shared = libcrux_ml_kem::mlkem768::decapsulate(&mlkem_signed_private, signed_ciphertext);
    ...
    if let Some(otk_id) = &initial_message.pq_one_time_key_id {
        ...
        if let Some(otk_private_bytes) = private_keys.pq_one_time_prekey_privates.get(otk_id) {
            let otk_private = MlKemPrivateKey::from_slice(otk_private_bytes)
                .context("Invalid ML-KEM one-time private key")?;
            let otk_shared = libcrux_ml_kem::mlkem768::decapsulate(&otk_private, otk_ciphertext);
            mlkem_outputs.push(otk_shared.to_vec());
        } else {
...
pub fn encrypt_pqxdh_session_message<R: CryptoRng + RngCore>(
    ...
    Ok(PqxdhSessionMessage {
        ...
        nonce: [0u8; 12], // This will be set by encrypt_with_shared_secret
        counter,
    })
}
...
pub fn decrypt_pqxdh_session_message(
    ...
    decrypt_with_shared_secret(&shared_secret.shared_key, &session_message.ciphertext)
}
