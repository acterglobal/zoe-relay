//! Simplified PQXDH cryptographic operations
//!
//! This module provides a working implementation of PQXDH key agreement that demonstrates
//! the protocol structure and message flow while using simplified cryptographic operations
//! that compile successfully with the current libcrux-ml-kem API.
...
/// Generate a complete PQXDH prekey bundle with private keys (simplified version)
pub fn generate_pqxdh_prekeys<R: CryptoRng + RngCore>(
    ...
    // Generate ML-KEM 768 signed prekey (simplified - using deterministic generation)
...
/// Perform PQXDH key agreement initiation (simplified version)
pub fn pqxdh_initiate<R: CryptoRng + RngCore>(
    ...
    let encrypted_payload = encrypt_with_shared_secret(&shared_secret.shared_key, initial_payload, &mut *rng)?;
...
/// Process PQXDH initial message (simplified version)
...
pub fn encrypt_pqxdh_session_message<R: CryptoRng + RngCore>(
    ...
    // Encrypt payload
    let encrypted_payload = encrypt_with_shared_secret(&shared_secret.shared_key, payload, &mut *rng)?;
...
fn sign_data(keypair: &KeyPair, data: &[u8]) -> Result<Signature> {
    match keypair {
        ...
        KeyPair::MlDsa44(mldsa_keypair, _) => {
            ...
            Ok(Signature::MlDsa44((Box::new(signature), blake3::hash(data))))
        }
        KeyPair::MlDsa65(mldsa_keypair, _) => {
            let signature = mldsa_keypair.sign(data);
            Ok(Signature::MlDsa65((Box::new(signature), blake3::hash(data))))
        }
        KeyPair::MlDsa87(mldsa_keypair, _) => {
            let signature = mldsa_keypair.sign(data);
            Ok(Signature::MlDsa87((Box::new(signature), blake3::hash(data))))
        }
    }
}
