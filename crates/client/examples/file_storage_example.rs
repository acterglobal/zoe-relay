...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   🗜️  Compressed: {}", stored_info.encryption_info.was_compressed);
    ...
    let raw_stored_info = file_storage
        .store_data(raw_data, "raw_data_example", Some("application/octet-stream".to_string()))
        .await?;
    
    println!("✅ Raw data stored!");
    println!("   📋 Blob hash: {}", raw_stored_info.blob_hash);
    println!("   📦 Original size: {} bytes", raw_stored_info.original_size);
    ...
    file_storage.retrieve_file_to_disk(&stored_info, &output_file).await?;
