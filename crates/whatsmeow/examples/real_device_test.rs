use whatsmeow::{WhatsAppBot, ConnectionStatus};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if connected {
            ...
            Ok(contacts) => {
                ...
                if contacts.len() > 0 {
                    ...
                    for (i, contact) in contacts.iter().take(3).enumerate() {
                        let name = contact.name.as_ref().map(|s| s.as_str()).unwrap_or("<No Name>");
            ...
            Ok(groups) => {
                ...
                if groups.len() > 0 {
                    ...
                    for (i, group) in groups.iter().take(3).enumerate() {
                        println!("      {}. {} ({} members)", i + 1, group.name, group.participants.len());
                    }
                    if groups.len() > 3 {
        ...
        if send_test.to_lowercase() == "y" {
            let recipient = prompt_user("   📱 Enter recipient (format: +1234567890@s.whatsapp.net): ");
            if !recipient.is_empty() && recipient.contains("@s.whatsapp.net") {
                let message = format!("🤖 Test message from Rust WhatsApp bot at {}", chrono::Utc::now().format("%Y-%m-%d %H:%M:%S UTC"));
