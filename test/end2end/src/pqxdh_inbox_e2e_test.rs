...
use zoe_wire_protocol::{
    ...
    StoreKey, StreamMessage, SubscriptionConfig, Tag, VerifyingKey, generate_keypair,
    inbox::pqxdh::{
        InboxType, PqxdhEchoServiceInbox, PqxdhInitialMessage, PqxdhPrekeyBundle,
        PqxdhSessionMessage, PqxdhSharedSecret,
        // Import the real crypto functions
        generate_pqxdh_prekeys, pqxdh_initiate, pqxdh_respond,
...
async fn test_pqxdh_types_and_serialization() -> Result<()> {
    ...
    let prekey_bundle = create_test_prekey_bundle(&generate_keypair(&mut rand::thread_rng()))?;
    ...
    assert_eq!(prekey_bundle.signed_prekey, deserialized_bundle.signed_prekey);
    assert_eq!(prekey_bundle.signed_prekey_id, deserialized_bundle.signed_prekey_id);
    assert_eq!(prekey_bundle.one_time_prekeys, deserialized_bundle.one_time_prekeys);
    assert_eq!(prekey_bundle.pq_signed_prekey, deserialized_bundle.pq_signed_prekey);
    assert_eq!(prekey_bundle.pq_signed_prekey_id, deserialized_bundle.pq_signed_prekey_id);
    assert_eq!(prekey_bundle.pq_one_time_keys, deserialized_bundle.pq_one_time_keys);
    ...
    assert_eq!(echo_inbox.pqxdh_prekeys.signed_prekey, deserialized_inbox.pqxdh_prekeys.signed_prekey);
    assert_eq!(echo_inbox.pqxdh_prekeys.signed_prekey_id, deserialized_inbox.pqxdh_prekeys.signed_prekey_id);
    assert_eq!(echo_inbox.pqxdh_prekeys.one_time_prekeys, deserialized_inbox.pqxdh_prekeys.one_time_prekeys);
    assert_eq!(echo_inbox.pqxdh_prekeys.pq_signed_prekey, deserialized_inbox.pqxdh_prekeys.pq_signed_prekey);
    assert_eq!(echo_inbox.pqxdh_prekeys.pq_signed_prekey_id, deserialized_inbox.pqxdh_prekeys.pq_signed_prekey_id);
    assert_eq!(echo_inbox.pqxdh_prekeys.pq_one_time_keys, deserialized_inbox.pqxdh_prekeys.pq_one_time_keys);
    ...
    let initial_message = PqxdhInitialMessage {
        initiator_identity: generate_keypair(&mut rand::thread_rng()).public_key(),
    ...
    let alice_keypair = generate_keypair(&mut rand::thread_rng());
...
async fn test_pqxdh_inbox_echo_service_e2e() -> Result<()> {
    ...
    let (prekey_bundle, alice_private_keys) = create_test_prekey_bundle_with_private_keys(alice.keypair())?;
    ...
    let pqxdh_message = Message::new_v0(
        ...
        Kind::Emphemeral(None), // Ephemeral message for RPC
        vec![
    ...
    let response_message = Message::new_v0(
        ...
        alice.public_key(),
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_secs(),
        Kind::Emphemeral(None), // Ephemeral response
        vec![
...
fn create_test_prekey_bundle_with_private_keys(keypair: &KeyPair) -> Result<(PqxdhPrekeyBundle, zoe_wire_protocol::inbox::pqxdh::PqxdhPrivateKeys)> {
    let mut rng = rand::thread_rng();
...
fn process_pqxdh_message(
    ...
    match pqxdh_content {
        zoe_wire_protocol::PqxdhEncryptedContent::Initial(initial_msg) => {
            ...
            let (decrypted_payload, _shared_secret) = pqxdh_respond(initial_msg, private_keys, prekey_bundle)?;
